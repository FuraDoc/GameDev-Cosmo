...
func refresh_items(hardware_layer: Control, clear_layer_callable: Callable) -> void:
	...
	for item_id in active_ids:
		...
		if data == null:
			push_error("Нет module visual data для item_id: '%s'" % item_id)
			continue

		var texture = load(data["texture_path"])
		if texture == null:
			push_error("Не удалось загрузить module текстуру: '%s'" % data["texture_path"])
			continue
...
func update_layer(hardware_layer: Control, cockpit_layer: Control, cockpit_texture: Texture2D) -> void:
	...
	for child in hardware_layer.get_children():
		...
		var data = module_visual_data.get(child.name, null)
		...
		var base_scale: float = min(
			hardware_layer.size.x / cockpit_texture.get_size().x,
			hardware_layer.size.y / cockpit_texture.get_size().y
		)
